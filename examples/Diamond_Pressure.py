...
pathbeg = filename[0:filename.rfind('/')] # The path where the raw and treated folders are
...
for lg in range(len(RTdiamond)):
    inputRT = np.genfromtxt(pathbeg+RTdiamond[lg],skip_header=20, skip_footer=43) # get the sample to deconvolute
    inputHT = np.genfromtxt(pathbeg+HTdiamond[lg],skip_header=20, skip_footer=43)
